...
한편, 동료 개발자들이 내 코드를 검토해준다는 점에서 이전과는 다르게 코드를 작성할 때 신중하게 생각하게 되었다. 더불어 코드 한 줄 한 줄에 대한 이유를 고민하고, 코드 품질 및 패턴에 대해 등한시하지 않고 고민하는 시간을 갖게 되었다. 현재 프로젝트에서는 이전에 없던 컴포넌트 패턴과 코드 구조를 개인적으로 도입하고 있는데, 이를 팀원들과 충분히 공유하고 설명하는 데에 노력하고 있다. (가끔은 이 부분에서 살짝 죄송한 감정이 들기도 한다..🥲)
