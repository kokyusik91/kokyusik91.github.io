...
스타트업에 입사하고 처음 맡게된 프로젝트는 **커뮤니티 게시판**이었다.하지만 일반적인 커뮤니티 게시판과 달랐던 점은 단순 웹사이트가 아닌 플러터 기반 앱에 들어가는 **웹뷰** 방식으로 만들어야 했다.<br> 개발자가 되고 처음으로 맡은 업무였고 프론트쪽은 주도적으로 혼자 담당해야 했기 때문에 긴장이 됬고, 문제없이 잘 만들어 나 자신을 어느정도 증명해야겠다는 생각에 열의가 넘쳤던 프로젝트였다.
...
기술에 대한 고민을 하지 않았다. 심지어 기존에 회사 내에 프론트엔드 개발 하시던 분이 없었어서 정해진 기술이 없었다. 즉 내가 최근에 배운 기술들을 사용하여 프로젝트를 혼자서 진행 하였다.
...
- 플러터의 네이티브 기능들 (imagePicker, 카메라, SnackBar )
- 기타 짜잘한 UX 기능들
...
기존에는 웹브라우저 환경에서만 개발을 했기 때문에, 전혀 문제가 없었지만... 이번에는 플러터로 만든 앱 내부에 들어갈 웹뷰였기 때문에 모바일에서 사용가능한 네이티브한 기능들을 이용할 수 있게 만들어줘야 했다.<br/> 추가로 네이티브 기능들을 호출하는 함수들을 웹 브라우저 개발 환경에서 호출하게 되면 에러가 발생한다. (예를들어 앱 내의 화면을 여는 버튼을 눌렀을때)

![플러터 함수 호출](/images/2022-12-25-eighth/flutterError.gif)

![플러터 함수 호출](/images/2022-12-25-eighth/flutter.png)
당연히 에러가 발생한다. 자바스크립트 코드에는 따로 callHandler라는 함수를 선언하지 않았기 때문. 단지 자바스크립트

## 해결 🤔?

애초에 웹 브라우저에는 이런 네이티브 기능을 제공하는 함수가 존재하지 않는다.<br> 플러터에서 선언한 함수들을 자바스크립트에서는 콜백함수 처럼 필요시점에 호출 하여 사용할 수 있도록 만들었다.
...
자바스크립트 코드에서 DIO라는 인스턴스를 만들었다. 웹브라우저에서 개발하는 환경에서는 Axios를 통한 요청, 응답이 이루어지게 하였고, 실제 스토어에 올라가는 웹뷰내에서는 플러터의 DIO 라이브러리가 서버에 직접 요청, 응답을 하도록 만들어 주었다.<br>
추가로 코딩 작업은 웹 브라우저에서 해야하기 때문에, web환경과 app환경을 애초에 분리시켜, web환경에서는 Axios를 사용한 통신, appView환경에서는 DIO를 사용한 통신으로 나눠서 개발하였다.<br>
